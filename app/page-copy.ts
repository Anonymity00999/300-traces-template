...
const defaults: Record<CopyLocale, Record<string, string>> = {
  zh: {
    ...
    eyebrow: "一份可被遇见的阅读档案", footprint: "你的阅读足迹", met: "已经与你相遇", held: "的痕迹已经被你接住", cloud: "足迹已在云端留下一份副本", next: "下一次靠近", all: "全部痕迹", status: "真实档案 · 可继续补入", open: "打开原文", read: "标记为读过", readAlready: "已经读过", draw: "遇见一条未读痕迹", note: "默认优先把尚未读过的痕迹交到你手里。", footer: "阅读是一种接住。", offer: "交来一条参考", passport: "取回 / 保存阅读凭证", original: "中文原文", translationNote: "保留原文；此浏览器暂不能生成英文译文。", translateLoading: "正在靠近英文…", source: "新的参考", labelIdea: "想法", labelArticle: "文章", labelAnswer: "回答", labelImage: "图片", labelEvent: "事件", labelAudio: "声音", labelFilm: "影像", labelLink: "入口", progressScope: "总进度包含档案文本、时间层图片、事件、声音、影像与入口；“遇见”不等于“读过”。",
    ...
    community: "实时新增", communityIntro: "已经接住的内容，会在这里出现。", linksTitle: "一些相遇", linksIntro: "个人作品与社区创作。", linksWork: "以斯帖的晨祷 · 个人作品", linksCommunity: "社区创作", linksOpen: "打开链接", linksEmpty: "链接还在等待被放下。", guestbook: "留下几句话", guestbookIntro: "不必署名。被审核接住以后，它会和后来到来的人相遇。", guestbookMessage: "想留下的话", guestbookName: "名字（选填）", guestbookAnonymous: "匿名", guestbookSend: "留下这句话", guestbookSending: "正在留下…", guestbookSent: "这句话已经收到，正在等待被接住。", guestbookEmpty: "还没有人把话留在这里。",
    periodTitle: "时间层 · 2026.03.16–22", periodHeading: "回到那一周", periodIntro: "", periodCount: "条痕迹", periodEmpty: "这一天还没有留下可显示的片段。", periodImageTitle: "图片轮动层", periodImageNote: "按日期轮动。", periodImageRandom: "遇见另一张图片", periodImageOpen: "打开图片来源", periodImageCredit: "来源 / 说明", periodImageEmpty: "这一天还没有图片。", periodImageSeen: "已经遇见",
...
export function defaultPageCopy(locale: CopyLocale, total: number) {
  ...
  copy.participationTitle = locale === "zh" ? "反馈" : "Feedback";
  copy.participationIntro = "";
  copy.participationReference = locale === "zh" ? "留下反馈" : "Leave feedback";
  copy.participationReferenceNote = locale === "zh" ? "不会进入公开档案。" : "It does not enter the public archive.";
  copy.linksRelated = locale === "zh" ? "活动链接" : "Activity links";
  copy.linksRelatedCommunity = locale === "zh" ? "相关整理" : "Related list";
  copy.submitTitle = locale === "zh" ? "反馈" : "Feedback";
  copy.submitIntro = locale === "zh" ? "你的反馈只会被项目维护者看见，不会进入公开档案。" : "Your feedback is only visible to the project maintainer; it does not enter the public archive.";
  ...
  copy.feedbackMessage = locale === "zh" ? "想说的话" : "What would you like to say?";
  copy.feedbackContact = locale === "zh" ? "联系方式（选填）" : "Contact details (optional)";
  copy.date = locale === "zh" ? "相关日期" : "Related date";
  copy.send = locale === "zh" ? "送出反馈" : "Send feedback";
  ...
  copy.consent = locale === "zh" ? "反馈不会公开显示。" : "Feedback is not displayed publicly.";
  copy.sent = locale === "zh" ? "反馈已经收到。" : "Your feedback has arrived.";
